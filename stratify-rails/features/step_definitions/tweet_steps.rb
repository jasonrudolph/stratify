Given /^a Twitter collector is configured for username "([^"]*)"$/ do |username|
  @collector = Stratify::Twitter::Collector.find_or_create_by(:username => username)
end
...
Then /^the most recent tweets from "jasonrudolph" should exist in the archive$/ do
  ...
  tweet = Stratify::Twitter::Activity.where(:status_id => 28832200464011265, :username => "jasonrudolph").first
  ...
  tweet = Stratify::Twitter::Activity.where(:status_id => 20595784953102338, :username => "jasonrudolph").first
  tweet.should_not be_nil
...
Then /^the archive should not include duplicate tweets$/ do
  grouped_tweets = Stratify::Twitter::Activity.only(:status_id).aggregate # => [{"status_id"=>26294541628, "count"=>1.0}, {"status_id"=>25131346543, "count"=>1.0}, ...]
